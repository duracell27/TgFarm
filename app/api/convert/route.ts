...
interface UpdateConvertType {
  userId:number, amount:number, type: "usd-1" | "usd-10" | "gold"
}
...
export const GET = async (req: NextRequest) => {
  ...
  const werehouse = await Convert.findOne({ userId });
...
export const PUT = async (req: NextRequest) => {
  ...
  try {
    ...
    if(type === 'usd-10'){
     ...
     return NextResponse.json({message: 'Converted 10 usd'}, { status: 200 });
   }
   ...
   if(type === 'gold'){
    ...
    await user.save();
    console.log('1')
    return NextResponse.json({message: 'Converted to usd'}, { status: 200 });
   }
